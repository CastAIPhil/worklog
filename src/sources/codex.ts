...
async function parseSessionDir(dirPath: string): Promise<WorkItem[]> {
		...
		for (const file of jsonlFiles) {
			...
			if (sessionStart && prompts.length > 0) {
				items.push({
					...
					metadata: { sessionFile: basename(file), promptCount: prompts.length },
				});
			}
		}
	} catch {
	...
	async read(dateRange: DateRange, config: Config): Promise<WorkItem[]> {
		...
		try {
			...
			for (const dir of sessionDirs) {
				const sessionItems = await parseSessionDir(dir);
